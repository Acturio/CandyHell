...
#* Endpoint para incorporar nuevos datos a postgres.
#* @post /new_data
...
#Endpoint regresa el AUC de la curva ROC del modelo.
#* @get /metrics
function(){
...
function(){
  ...
  plot <- autoplot(roc_curve_cla_logistico) + ggtitle("ROC curve")
  file <- "curva_roc.png"
  ggsave(file, plot)
...
#* Endpoint de grafica PRECISION_RECALL
#* @serializer contentType list(type='image/png')
#* @get /pr_curve_plot
function(){
  model <- readRDS("logistic_model.Rds")
  ...
  plot2 <- autoplot(pr_curve_cla_logistico) + ggtitle("Precision-recall")
  file2 <- "pr_plot.png"
  ggsave(file2, plot2)
